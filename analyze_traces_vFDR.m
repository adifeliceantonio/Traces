...
nl_pvalue=zeros(nl_totaltime,1);
...
for i=1:nl_totalrois-1
    ...
    for contrast=1:2
        ...
        nl_pvalue(i,1)=coefTest(nl_lms.(nl_modelname), H);
end
[nl_FDR, nl_Q]=mafdr(nl_pvalue);
for p=1:nl_totalrois-1
    if nl_Q(p,1)<0.01
        if roi_means.(nl_contrastnames{1,1})(p,1)> roi_means.(nl_contrastnames{1,2})(p,1)
        nl_results.(nl_columnames{1})(p,1)=1;
        else
        nl_results.(nl_columnames{2})(p,1)=1;
        end      
    end
end
...
acq_pvalue=zeros(acq_totaltime,1);
...
for i=1:acq_totalrois-1
    ...
    for contrast=1:2
        ...
        acq_pvalue(i,1)=coefTest(acq_lms.(acq_modelname), H);
end
[acq_FDR, acq_Q]=mafdr(acq_pvalue);
for i=1:acq_totalrois-1
    if acq_Q(i,1)<0.01
...
disp('Saving Data')
save_filenameacq=sprintf('%s_ACQ_results_FDR.mat', rat_id);
save(save_filenameacq, 'acq_results', 'acq*', '-v7.3');
save_filenamenl=sprintf('%s_NL_results_FDR.mat', rat_id);
